...
deepspeed --include localhost:0,1,2,3 --master_port 29512 gelm/train/train_mem.py \
    ...
    --bf16 True \
    --output_dir ./finetuned/${DATASET}_${RUN_NAME} \
    --num_train_epochs 5 \
    ...
    --report_to none \
    --run_name $RUN_NAME \
    --tasks "temporal_reasoning||multihop_qa" \
    --temporal_reasoning_data "activitynet" \
    --temporal_reasoning_sample_rate 1 \
    --multihop_qa_data "ego4d" \
    --multihop_qa_sample_rate 1 \
    --task_sample_rate 3 1 \
    --input_type feature \
    --num_frames 180 \
    --feature_dim 768 \
    --gnd_enc_layers 1 \
